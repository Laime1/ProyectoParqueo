...
class Parqueo1Controller extends Controller
{
    public function asignarPuesto(Request $request) {
        ...
        DB::update("UPDATE parqueo1 SET cliente_sis=$sis, cliente_secundario=$sis2Value, Estado='1', color='green', fecha_inicio='$inicio'
                 WHERE numero=$puesto ");
        }elseif($accion === 'vaciar sitio'){
            DB::update("UPDATE clientes SET Puesto=null WHERE CodigoSIS=$sis ");
            DB::update("UPDATE parqueo1 SET cliente_sis=null, Estado='0', color='gray', fecha_inicio=null
              WHERE numero=$puesto ");
        } 
